    ...
    public partial class clinic_doctor_service : Form
    {
        ...
        public clinic_doctor_service()
        {
            ...
            foreach (DataRow item in dt.Rows)
            {
                ...
                dataGridView2.Rows[n].Cells[4].Value = item["medi_qty_type"].ToString();
            }
...
/*
                ...
                if (sdr.Read())
                {
                    ...
                    if (unit_medi == "เม็ด")
                    {
                        if (cut_stock == 0  && nummed >= 5)
                        {
                            ...
                            MessageBox.Show("บันทึกการจ่ายยาเรียบร้อย");
        ...
        {
            conn.Open();
            string query = ("select count(*) from treatment_record where treatr_id = '" + lblidt.Text + "'");
            cmd = new SqlCommand(query, conn);
            sda = new SqlDataAdapter(cmd);
            dt = new DataTable();
            sda.Fill(dt);
