          ...
          try
           {
                ...
                if (time >= 08.00 && time <= 12.00)
                {
                  //  MessageBox.Show("เช้า");sadsdasdsafsaไกดหหหหหหหหdededededหหหดำไหด
                    ...
                    SqlCommand cmd1 = new SqlCommand("select employee_doctor.emp_doc_name,schedule_work_doctor.swd_day_work,schedule_work_doctor.swd_date_work,schedule_work_doctor.room_id,time_attendance.remark from employee_doctor inner join user_control on user_control.emp_doc_id = employee_doctor.emp_doc_id inner join schedule_work_doctor on schedule_work_doctor.emp_doc_id = employee_doctor.emp_doc_id  inner join time_attendance on time_attendance.emp_doc_id = employee_doctor.emp_doc_id where uct_user=@uct_user and uct_password=@uct_password and swd_day_work = '" + day+ "' and schedule_work_doctor.swd_date_work = '" + today1 + "' and swd_timezone = 'เช้า' and time_attendance.remark = 'เข้างาน'  ORDER BY id_time DESC", conn);
                    conn.Open();
